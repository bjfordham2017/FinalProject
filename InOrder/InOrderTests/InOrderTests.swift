...
//    }
    ...
    func testNewUserJSON() {
        let user = User()
        let userToJSON = user.jsonObject
        let userFromJSON = User(jsonOjbect: userToJSON)!
        XCTAssertEqual(user.name, userFromJSON.name)
        XCTAssertEqual(user.email, userFromJSON.email)
        XCTAssertEqual(user.id, userFromJSON.id)
        XCTAssertEqual(user.groupDirectory[0].id, user.groupDirectory[0].id)
    }
